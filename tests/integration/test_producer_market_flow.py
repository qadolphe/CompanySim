...
def env() -> PolicySnapshot:
    return PolicySnapshot(
        ...
        emissions_penalty_per_unit=200, cafe_ev_mandate_pct=0.15,
    )
...
class TestProducerMarketFlow:
    ...
    def test_automaker_posts_and_reads_sales(
        ...
        for _ in range(180):
            marketplace.attempt_purchase("HYBRID")
        for _ in range(100):
            marketplace.attempt_purchase("EV")

        # 3. Get sales summary
        sales = marketplace.get_sales_summary()
    ...
    def test_ev_sellout_triggers_capacity_increase(
        ...
        sales = {
            "ICE": SalesRecord("ICE", 150, 4_800_000),    # 25% of 600
            "HYBRID": SalesRecord("HYBRID", 180, 6_300_000),  # 72%
            "EV": SalesRecord("EV", 145, 6_090_000),       # 97% of 150
        }
        automaker.process_sales(sales, env)
    ...
    def test_ice_underperformance_triggers_decrease(
        ...
        sales = {
            "ICE": SalesRecord("ICE", 150, 4_800_000),  # 25% of 600
            "HYBRID": SalesRecord("HYBRID", 180, 6_300_000),
            "EV": SalesRecord("EV", 148, 6_216_000),  # 98.7%
        }
        automaker.process_sales(sales, env)
    ...
    def test_multi_tick_stability(self, env: PolicySnapshot) -> None:
        ...
        for i in range(5):
            sales = {
                "ICE": SalesRecord("ICE", 400, 400 * 32_000),
                "HYBRID": SalesRecord("HYBRID", 200, 200 * 35_000),
                "EV": SalesRecord("EV", 140, 140 * 42_000),
            }
            automaker.process_sales(sales, env)
